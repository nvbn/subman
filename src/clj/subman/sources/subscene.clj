...
(defn- make-url
  ...
  [url] (str "http://subscene.com" url))

(defn- get-page-url
  "Get release page url"
  [page] (make-url (str "/browse/latest/series/" page)))


(defn- remove-spec-symbols
  "Remove spec symbols"
  [text] (clojure.string/replace text #"[\t\n]" ""))

(defn- get-version
  "Get version from page"
  [page] (->> (html/select page [:li.release :div])
              (map #(-> %
                        :content
                        first
                        remove-spec-symbols))
...
(defn- get-lang
  "Get lang from page"
  [page] (->> (html/select page [:div.download :a])
              first
              :content
              first
...
(defn- create-subtitle
  ...
  [url] (let [page (helpers/fetch url)
